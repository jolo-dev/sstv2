	...
	try {
		...
		const output = stacks[0].Outputs?.forEach((o) => {
			if (o.OutputKey === "BootstrapVersion") {
				version = Number.parseInt(o.OutputValue!);
...
async function loadSSTStatus() {
	...
	const latestMajor = Number.parseInt(latestParts[0]);
	const latestMinor = Number.parseInt(latestParts[1] || "0");
	const currentParts = version.split(".");
	const currentMajor = Number.parseInt(currentParts[0]);
	const currentMinor = Number.parseInt(currentParts[1] || "0");
	if (
